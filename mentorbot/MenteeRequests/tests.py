...
class ModelTestCase(TestCase):
    ...
    def setup(self):
        self.menteerequestnewmentorshipfield = {
            'mentee_name': 'Mary Jane', 'requested_mentorship_field': 'python',
            'request_status': False}
        self.menteerequestnewmentorshipfield_missingparameters = {
            'mentee_name': 'Mary Jane', 'requested_mentorship_field': '',
            'request_status': False}
        self.menteerequestnewmentorshipfield_duplicateentry = {
            'mentee_name': 'Mary Jane', 'requested_mentorship_field': 'python',
            'request_status': False}
        self.menteerequestNewMentorship = MenteeRequests(
            self.menteerequestnewmentorshipfield)
        self.menteerequestNewMentorship_MissingParameters = MenteeRequests(
            self.menteerequestnewmentorshipfield_missingparameters)
        self.menteerequestNewMentorship_DuplicateEntry = MenteeRequests(
            self.menteerequestnewmentorshipfield_duplicateentry)
...
class ViewTestCase(TestCase):
    ...
    def setup(self):
        ...
        self.response1 = self.client.post(
            self.menteerequestnewmentorshipfield, format="json")
        self.response2 = self.client.post(
            self.menteerequestnewmentorshipfield_missingparameters,
            format="json")
        self.response3 = self.client.post(
            self.menteerequestnewmentorshipfield_duplicateentry, format="json")

    def test_adding_new_mentee_request(self):
        self.assertEqual(self.response1.status_code, status.HTTP_201_CREATED)

    def test_missing_parameters_in_mentee_request(self):
        self.assertEqual(self.response1.status_code,
